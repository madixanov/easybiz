            ...
            <table>
                ...
                <tbody>
                    <tr v-for="(page) in projects">
                        ...
                        <td>
                            <a target="_blank" :href="`http://${(page as any).id}.landing.easybiz.uz/${(page as any).urlPath}`">
                                {{ (page as any).urlPath ? (page as any).urlPath : 'Путь не подключен' }}
                            </a>
                        </td>
                        <td> {{ (() => {
                            const date = new Date((page as any).lastSaved);
                        ...
                        <td v-if="(page as any).isPublished"><span class="status good">Опубликован</span></td>
                        ...
                        <td align="center" style="display: flex; align-items: center; font-size: 24px; height: 100%;">
                            ...
                            <ul class="table-dropdown">
                                <li>
                                    <nuxt-link
                                        :to="`/projects/${$router.currentRoute.value.params.project_id}/editor/${(page as any).id}`">Редактировать</nuxt-link>
                                </li>
                            </ul>
                        </td>
                    </tr>
