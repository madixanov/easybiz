<template>
   <div class="dash-container" role="container">
      ...
      <div class="dash-container-column">
        <UiLoader :height="'100%'" :has-background="false" v-if="!loaded" :style="{position: 'relative', height: '20rem'}" />
        <CategoryList :cat="categories.map((e: any) => ({ title: e.category_name }))" :title="'прибыльные категории'" :style="{
        ...
        <UiLoader :height="'100%'" :has-background="false" v-if="!loaded" :style="{position: 'relative', height: '20rem'}" />
        <ProductsList :prod="products" :title="'просматриваемые продукты'" :height="35" :style="{
                height: loaded ? 'auto' : '0',
                opacity: loaded ? 1 : 0,
                transition: 'opacity 500ms ease',
                transitionDelay: '100ms'
      ...
      <div class="dash-container-column">
        <UiLoader :height="'100%'" :has-background="false" v-if="!loaded" :style="{position: 'relative', height: '20rem'}" />
        <ProductsList :prod="products" :title="'продаваемые продукты'" :height="35" :style="{
                height: loaded ? 'auto' : '0',
                opacity: loaded ? 1 : 0,
                transition: 'opacity 500ms ease',
                transitionDelay: '100ms'
        ...
        <UiLoader :height="'100%'" :has-background="false" v-if="!loaded" :style="{position: 'relative', height: '20rem'}" />
        <Analytics :length="products.length" :style="{
                height: loaded ? 'auto' : '0',
                opacity: loaded ? 1 : 0,
                transition: 'opacity 500ms ease',
                transitionDelay: '100ms'
